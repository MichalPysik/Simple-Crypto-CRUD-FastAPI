...
class CryptocurrencyBase(BaseModel):
    ...
    amount: Optional[float] = Field(
        None,
        description="Amount of cryptocurrency owned",
        ge=0.0,
    )
...
class CryptocurrencyUpdate(BaseModel):
    ...
    # Only name and amount can be updated
...
class CryptocurrencyMetadata(BaseModel):
    ...
    metadata_timestamp: Optional[datetime] = Field(
        None,
        description="Datetime fetched from Coingecko API, corresponding to the actuality of the data",
    )
...
class CryptocurrencyResponse(Cryptocurrency):
    ...
    crypto_metadata: Optional[CryptocurrencyMetadata] = None
